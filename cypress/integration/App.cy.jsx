import { DarkTheme, LightTheme, UNCORRECT_BRACKETS_MESSAGE, UNCORRECT_INPUT_MESSAGE, UNCORRECT_OPERATOR_MESSAGE, WARNING_MESSAGE_TIMEOUT } from '@/constants';
...
describe('Expressions and Display', () => {
  ...
  it('check correct division functional', () => {
    ...
    cy.getBySel('display').should('have.text', '2.020');
    cy.getBySel('C').click();
  });
...
describe('Brackets', () => {
  ...
  it('check uncorrect brackets input two', () => {
    ...
    cy.getBySel('display').should('have.text', UNCORRECT_BRACKETS_MESSAGE);
    cy.wait(WARNING_MESSAGE_TIMEOUT);
    cy.getBySel('display').should('have.text', ' ( 56 + 88');
    cy.getBySel('C').click();
  });
  it('check uncorrect brackets input three', () => {
    ...
    cy.getBySel('display').should('have.text', UNCORRECT_BRACKETS_MESSAGE);
    cy.wait(WARNING_MESSAGE_TIMEOUT);
    cy.getBySel('display').should('have.text', ' (  (  ( 60 / 6 ) ');
    cy.getBySel('C').click();
  });
});
...
describe('History', () => {
  it('initial history state has to be empty', () => {
    cy.visit('http://localhost:3030/');
    cy.window().its('store').invoke('getState').its('main').its('history').should('have.length', 0);
  });
  ...
  it('adding items to history', () => {
    ...
    cy.window().its('store').invoke('getState').its('main').its('history').should('have.length', 1);
  });
...
describe('Application theme', () => {
  ...
  it('change theme from light to dark', () => {
    cy.getBySel('settings').click();
    cy.getBySel('theme_button').click();
    cy.getBySel('Dark theme' || 'Темная тема').click();
    cy.getBySel('home').click();
  ...
  it('change theme from dark to light', () => {
    cy.getBySel('settings').click();
    cy.getBySel('theme_button').click();
    cy.getBySel('Light theme' || 'Светлая тема').click();
    cy.getBySel('home').click();
    cy.window().its('store').invoke('getState').its('theme').should('is', LightTheme);
  });
});
